...
<template>
  ...
  <div v-if="this.$store.state.themeObj.user_start!=0" class="footBack footBackQian">
    <div
      class="footer-img"
      :style="{background:this.$store.state.themeObj.bottom_img?'url('+this.$store.state.themeObj.bottom_img+') no-repeat 50%':'url(/img/footer01.png) no-repeat 50%'}"
    ></div>
    <div class="fcontainer">
      ...
      <p>
        托管于
        <a href="https://github.com/omheshigenwozou" target="_blank">GitHub</a>.
      ...
      <p>
        <span>Made with</span>
        <img class="fheart" src="/img/heart02.png" />
        <span>by Aimee.</span>
      </p>
    </div>
  </div>
